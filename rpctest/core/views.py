...
traffic_coefficient = [1.036, 1.015, 1.004, 1, 1.003, 1.018, 1.066, 1.190, 1.254, 1.244, 1.221, 1.218, 1.218, 1.222,
                       1.221, 1.216, 1.226, 1.228, 1.241, 1.232, 1.210, 1.171, 1.130, 1.076]
...
class MakeModeling(Service):
    ...
    def vrp_modeling(ctx, school, bus_types, max_route_time, direction, time, school_time, service_time, modeling_time,
                     points):
        solution = {}
        solution['info'] = {}
        gotomodeling = 0
        ...
        try:
            ...
            # Create data for modeling route for business with defined all points
                ...
                data = create_data_model_date_range(data_input)
            else:
                data = {}
        ...
        if gotomodeling == 1:
            ...
            if school['form'] == 0:
                ...
                # If not, then we raise various errors related to the solver
                    ...
                    solution = {
                        'status': {'code': 'UnknownSolverERROR', 'message': 'Error desconocido con solucionador'}}
                ...
                for key in data['school_time'].keys():
                    ...
                    if solver_result['status'] == 1:
                        ...
                        # If defined start and finish:
                            ...
                            local_solution.append(
                                {key: print_solution_empresa_route(data, solver_result, data_input, key)})
            ...
            # Modeling for empresa en daterage (1 modeling for all points and transpose to all days)
            elif school['form'] == 4:
                solver_result = main(data, 'all')

                # If a solution was found, then output
                if solver_result['status'] == 1:
                    solution = print_solution_date_range(data, solver_result, data_input)
                    solution['status'] = {'code': 'OK', 'message': 'Problema resuelto con éxito'}
                # If not, then we raise various errors related to the solver
                elif solver_result['status'] == 2:
                    solution = {'status': {'code': 'NoSolutionERROR',
...
# Function to translate data from json to solver format (form=0, form=1)
def create_data_model(data_input):
    ...
    if result['max_time'] >= data['max_route_time']:
        data[
            'info'] = 'El tiempo de viaje desde el inicio hasta uno de los puntos finales es mayor que el tiempo máximo de viaje. '
    ...
    for key in data['school_time'].keys():
        ...
        if total_demands > data['total_capacity']:
            data[
                'info'] = 'Necesidad total es más que la capacidad total. No se encontrará la solución. Verifique el número de autobuses o auméntelo'
...
# Function to translate data from json to solver format for route with defined start and finish (form=2)
    ...
    if data['direction'] == 0:
        ...
        for a in data_input['points']:
            ...
            if index == 0:
                ...
                for key in data['school_time'].keys():
                    data['time_windows'][key].append([0, data['max_route_time']])
                    data['dict_coordinates'][key] += str(data_input['school']['lng']) + ',' + str(
                        data_input['school']['lat']) + ';'
                    data['points_info'][key].append(
                        {'coordinates_for_route': str(data_input['school']['lng']) + ',' + str(
                            data_input['school']['lat']),
                         'id': data_input['school']['id'],
                         'presence': {key: 1}})
...
# Function to translate data from json to solver format for route with defined start and finish (form=3)
...
# Function to translate data from json to solver format (form=4)
def create_data_model_date_range(data_input):
    # Create a template
    data = {}
    data['time_coefficient'] = traffic_coefficient[data_input['time']]
    data['time'] = data_input['time']
    ...
    for key in data['school_time']['dates']:
        data['time_windows'][key] = []
        data['points_info'][key] = []
        data['days'].append(key)
        presence.update({key: 1})
    ...
    if data['direction'] == 0:
        ...
        for a in data_input['points']:
            ...
            presence = {}
            for key in data['school_time']['dates']:
                presence.update({key: a['dates'][key]})
                if a['dates'][key] == 1:
                    data['dict_coordinates'][key] += coord + ';'
                    data['time_windows'][key].append([twstart, data['max_route_time']])
                    data['points_info'][key].append(
...
# Output the result in the required format for schools (form=0)
...
# Output the result in the required format for empresas (form=1)
...
# Output the result in the required format for route with defined start and finish (form=2)
...
# Output the result in the required format for route with defined start and finish (form=3)
def print_solution_empresa_route_defined(data, key):
    ...
    if data['direction'] == 0:
        for a in info['draw']['waypoints']:
            ...
            if passanger_time > max_time and data['points_info'][key][ind]['id']['point_id'] != 0 and \
                    data['points_info'][key][ind]['id']['person_id'] != 0:
    ...
    if len(error_pass_log) > 0:
        warning.append({'code': 'MaxTimeErrorPassenger',
                        'message': 'Excedido el tiempo máximo de viaje para siguientes pasajeros:' + error_pass_log})
...
# Output the result in the required format for schools (form=4)
def print_solution_date_range(data, solver_result, data_input):
    manager = solver_result['manager']
    routing = solver_result['routing']
    solution = solver_result['solution']
    serv_result = {}
    ...
    for key in data['school_time']['dates']:
        ...
        day_time = data['school_time']['time']
